...
export const setGoalTracked = async ({
  ...
  const goal = {
    ...
    reminder:
      reminder !== undefined ? reminder : existingGoal?.reminder ?? null,
  };
...
export const updateApiReminer = async ({
  id,
  daysOfWeek,
  enabled,
  reminder,
...
export const getGoalsTracked = async (
  { date, enabled = true } = { date: undefined, enabled: true }
) => {
...
export const setGoalDailyRecord = async ({ goalId, value, comment, date }) => {
  ...
  const existingRecord = existingRecordId
    ? data.records.data[existingRecordId]
    : undefined;
  ...
  data = {
    ...
    records: {
      ...
      byDate: {
        ...data.records?.byDate,
        [date]: [
          ...(data.records?.byDate?.[date]?.filter?.((_id) => _id !== id) ||
            []),
          id,
        ],
      },
      byGoalId: {
        ...
        [goalId]: [
          ...(data.records?.byGoalId?.[goalId]?.filter?.((_id) => _id !== id) ||
            []),
          id,
        ],
      },
...
export const getGoalsDailyRecords = async (
  { date, goalId } = { date: undefined, goalId: undefined }
) => {
  let data = await getData();
...
export const getDaysOfWeekLabel = (daysOfWeek) => {
  ...
  if (_daysOfWeek.length === 7) return "all";
    ...
    return _daysOfWeek
      ...
      .map(({ day, index }) =>
        format(setDay(new Date(), index), "eee", { locale: fr })
      )
